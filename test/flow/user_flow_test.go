...
func TestUserFlow_MeAndAdminManagement(t *testing.T) {
	...
	meRes := authedRequest(t, http.MethodGet, ts.URL+"/api/v1/me", fabio, nil)
	if meRes.StatusCode != http.StatusOK {
		t.Fatalf("GET /me: got status %d", meRes.StatusCode)
	}
	me := decodeJSON[meResponse](t, meRes)
	if me.Username != "fabio" || !me.IsAdmin || me.Disabled || me.StorageUsedBytes != 0 {
		t.Errorf("/me = %+v, want username=fabio is_admin=true disabled=false storage_used_bytes=0", me)
	}
	...
	forbiddenPatchRes := authedRequest(t, http.MethodPatch, ts.URL+"/api/v1/users/"+fabio.id, mario, map[string]any{"disabled": true})
	...
	listRes := authedRequest(t, http.MethodGet, ts.URL+"/api/v1/users", fabio, nil)
	...
	quotaPatchRes := authedRequest(t, http.MethodPatch, ts.URL+"/api/v1/users/"+mario.id, fabio, map[string]any{"quota_bytes": newQuota})
	...
	disablePatchRes := authedRequest(t, http.MethodPatch, ts.URL+"/api/v1/users/"+mario.id, fabio, map[string]any{"disabled": true})
...
func TestInviteFlow_AdminCreatesInviteViaAPI(t *testing.T) {
	...
	quota := int64(999999)
	createRes := authedRequest(t, http.MethodPost, ts.URL+"/api/v1/invites", fabio, map[string]any{"quota_bytes": quota})
