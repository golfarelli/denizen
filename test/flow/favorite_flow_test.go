...
func TestFavoriteFlow_AddListRemove(t *testing.T) {
	...
	item := uploadFile(t, ts, alice, nil, "favorite-me.txt", []byte("star this"))

	if res := addFavorite(t, ts, alice, item.ID); res.StatusCode != http.StatusNoContent {
		t.Fatalf("add favorite: got status %d", res.StatusCode)
	}

	favorites := listFavorites(t, ts, alice)
	...
	if favorites := listFavorites(t, ts, alice); len(favorites) != 1 {
	...
	listRes := authedRequest(t, http.MethodGet, ts.URL+"/api/v1/items", alice, nil)
	...
	if res := removeFavorite(t, ts, alice, item.ID); res.StatusCode != http.StatusNoContent {
		t.Fatalf("remove favorite: got status %d", res.StatusCode)
	}
	if favorites := listFavorites(t, ts, alice); len(favorites) != 0 {
...
// scope decision: favoriting isn't
...
func TestFavoriteFlow_SharedItemIncludedUntilRevoked(t *testing.T) {
	...
	item := uploadFile(t, ts, alice, nil, "shared-with-mario.txt", []byte("shared content"))
	grantRes := createUserShare(t, ts, alice, item.ID, mario.id)
	...
	if res := addFavorite(t, ts, mario, item.ID); res.StatusCode != http.StatusNoContent {
		t.Fatalf("mario favorites alice's shared item: got status %d", res.StatusCode)
	}
...
func TestFavoriteFlow_CannotFavoriteSomeoneElsesPrivateItem(t *testing.T) {
	...
	item := uploadFile(t, ts, alice, nil, "private.txt", []byte("not shared with anyone"))

	if res := addFavorite(t, ts, mario, item.ID); res.StatusCode != http.StatusNotFound {
		t.Errorf("mario favoriting alice's private item: got status %d, want %d", res.StatusCode, http.StatusNotFound)
	}
}
